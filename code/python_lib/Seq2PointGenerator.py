...
class Seq2PointGenerator(keras.utils.Sequence):
    def __init__(self, df, windowsize=599, batchsize=1000,  shuffle=True):
        ...
        self.max_batchsize = self.inputs.size - 2 * self.offset

    def __len__(self):
        a = int(np.ceil(len(self.inputs)/self.batchsize))
        return a
    ...
    def __getitem__(self, index):
        ...
        for start_idx in range(0, self.max_batchsize, self.batchsize):
            ...
            # of data.
            #
            tar = self.targets[excerpt + self.offset].reshape(-1, 1)
...
#
    ...
    #     self.offset = int(0.5*(self.windowsize-1.0))
    #     self.max_batchsize = self.inputs.size - 2 * self.offset
    #     self.indexes = np.arange(self.max_batchsize)
